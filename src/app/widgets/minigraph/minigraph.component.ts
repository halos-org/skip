...
import { IGraphDatapoint, TimeScaleFormat } from '../../core/interfaces/graph-data.interfaces';
...
const MINIGRAPH_TIME_SCALE: TimeScaleFormat = 'minute';
const MINIGRAPH_PERIOD = 0.2;

@Component({
  selector: 'minigraph',
  imports: [],
  templateUrl: './minigraph.component.html',
  styleUrls: ['./minigraph.component.scss'],
...
export class MinigraphComponent implements OnDestroy {
  ...
  public startGraph(): void {
    ...
    const windowMs = resolveWindowMs(MINIGRAPH_TIME_SCALE, MINIGRAPH_PERIOD);
    ...
    this.setChartOptions();
    this.createDatasets();
    const canvasEl = this.widgetDataGraph()?.nativeElement as HTMLCanvasElement | undefined;
    ...
    if (this.verticalChart) {
      this.lineChartOptions.scales = {
        ...
        y: {
          ...
          time: {
            unit: MINIGRAPH_TIME_SCALE as TimeUnit,
            minUnit: "second",
            round: "second",
            displayFormats: {
    ...
    this.lineChartOptions.plugins = {
      legend: {
       ...
       streaming: {
        ...
        frameRate: MINIGRAPH_TIME_SCALE === "day" ? 5 : MINIGRAPH_TIME_SCALE === "hour" ? 8 : MINIGRAPH_TIME_SCALE === "minute" ? 15 : 30,
       }
    }
  }
  ...
  private startStreaming(): void {
    ...
    const params: IHistoryGraphStreamParams = {
      ...
      windowMs: resolveWindowMs(MINIGRAPH_TIME_SCALE, MINIGRAPH_PERIOD),
    ...
    this.streamSub = this.historyStream.getBackfillThenLive(params).subscribe(emission => {
      ...
      if (Array.isArray(emission)) {
        // Initial backfill: fill the graph with the window's points.
        const valueRows = this.transformSeriesRows(emission, 0);
        this.chart.data.datasets[0].data.push(...valueRows);
        if (this.config.showAverageData) {
          const avgRows = this.transformSeriesRows(emission, this.config.datasetAverageArray);
          this.chart.data.datasets[1].data.push(...avgRows);
        }
      } else {
        // Live: handle new single datapoint
        const valueRow = this.transformSeriesRows([emission], 0)[0];
        this.chart.data.datasets[0].data.push(valueRow);

        if (this.config.showAverageData) {
          const avgRow = this.transformSeriesRows([emission], this.config.datasetAverageArray)[0];
