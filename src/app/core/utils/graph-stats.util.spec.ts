...
import { circularMeanRad, circularMinMaxRad, computeWindowStats, normalizeDirectionRad, normalizeSignedRad } from './graph-stats.util';

describe('graph-stats.util', () => {
  ...
  it('normalizes exactly π to +π in the signed domain (atan2 boundary, shared by both engines)', () => {
    ...
    // (-π, π]), not the recorder's former mod-based -π. Both graph engines now agree here.
