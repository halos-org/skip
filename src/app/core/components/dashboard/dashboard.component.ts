...
export class DashboardComponent implements AfterViewInit, OnDestroy {
  ...
  /** Any session whose edits could never be saved — the chromeless embed, an anonymous read-only
   *  visitor, a signed-in `readonly` user. Gates the empty-state "Unlock and Customize" button,
   *  which calls into a lock the same predicate refuses to open. */
  ...
  /** True while a slide is animating; blocks re-entrant runPageChange so overlapping changes don't fight. */
  private _slideInFlight = false;
  /** Set in ngOnDestroy so a slide in flight bails out instead of touching a torn-down grid. */
  private _destroyed = false;
  /** The current phase animation, kept so ngOnDestroy can cancel it. */
  ...
  ngAfterViewInit(): void {
    ...
    this.dashboard.widgetAction$.pipe(takeUntilDestroyed(this._destroyRef)).subscribe((action: widgetOperation) => {
      if (action) {
        ...
        if (!grid) return;
        grid.getGridItems().forEach((item: GridItemHTMLElement) => {
   ...
   */
  private loadDashboard(dashboardId: number | null): void {
    // Null (before the first route resolves) or out-of-range index: nothing to load.
   ...
   * during a slide), settling on the latest target when the slide ends. This
   * keeps the grid from ever diverging from the URL, and the service transition
   * flag makes the animated slide non-interruptible from navigate* entry points.
   */
  private async runPageChange(): Promise<void> {
    // A slide is already running; it will settle on the latest activeDashboard.
    if (this._slideInFlight) return;
    ...
    try {
      ...
      if (this.dashboard.activeDashboard() !== target) {
        target = this.dashboard.activeDashboard();
        this.loadDashboard(target);
      }
    } finally {
  ...
  protected saveDashboard(): void {
    ...
    if (!grid || activeDashboard === null) return;
    const serializedData = grid.save(false, false) as NgGridStackWidget[] || null;
  ...
  protected onEmptyAreaTap(e: Event | CustomEvent): void {
    ...
    const grid = this._gridstack().grid;
    if (!grid) return;
    const gridCell = grid.getCellFromPixel({ left: inputX, top: inputY });
    if (!grid.isAreaEmpty(gridCell.x, gridCell.y, 1, 2)) {
      this.toast.show('Not enough free space at the selected location to add a widget. Please reorganize the page to free up space or try a larger empty area.', 0, false, 'error');
      return;
    }
  ...
  private addWidgetToGrid(widget: WidgetDescription, x: number, y: number): void {
    ...
    if (!defaultFits) {
      ...
      if (!best) {
        this.toast.show(`Not enough free space to add "${widget.name}" widget at the selected location. Please reorganize the page to free up space or try a larger empty area.`, 0, false, 'error');
        return;
      }
    ...
    const item = grid.addWidget(newWidget);
    const node = item.gridstackNode;
    if (node?.subGrid && node.subGridOpts) {
      node.subGridOpts.row = node.subGridOpts.minRow = node.subGridOpts.maxRow = node.h; // Ensure subgrid rows match initial height
    }
  }
  ...
  private duplicateWidget(item: GridItemHTMLElement): void {
    const node = item.gridstackNode;
    if (!node) return;
    const source: NgGridStackWidget = node;
    const widgetProps = source.input?.widgetProperties;
    ...
    if (grid.willItFit(newItem)) {
      ...
      if (grid.willItFit(newItem)) {
        ...
        this.toast.show('Insufficient space on the page. Please reorganize to free up space.', 0, false, 'error');
      }
    }
  }
  ...
  ngOnDestroy(): void {
    // Tear down any slide in flight: bail the async sequence out and clear the
    // shared transition flag so a torn-down component can never wedge navigation.
