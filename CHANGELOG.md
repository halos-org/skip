...
## Fixes
* A new Compact Linear gauge starts unfiltered, so its Signal K Path list offers every path rather than only the voltage ones. It shipped with its unit filter preset to **Electrical: Volt**, which is what every other general-purpose gauge leaves at **All**, and its scale preset to 0–15 to match. New widgets now start at **All** and 0–100; gauges you have already placed keep the settings you gave them. The Compact Linear gauges on the pages Skip ships — fuel level and engine temperatures — carried that same **Electrical: Volt** filter in their own saved settings, so on a fresh install their Path lists were empty of the very paths they were bound to; those start at **All** now as well. Fixes #541
* Fuel rate, and every other reading whose unit your Signal K server names as `L/h`, `km/h`, `F`, kilometers, miles or feet, now shows in that unit instead of a raw SI number labelled "unitless" — a fuel rate reads 7.2 l/h where it used to read 0.00. Skip takes each path's display unit from the server's unit preferences, but recognised only five of the names the server actually publishes and gave up on the rest without saying so. Every unit the six built-in presets select is recognised now, along with the spellings a per-path override can choose (`L/min`, `meter`, `radian`, `hertz`, `second`, and the rest), and Skip gained the imperial gallon, imperial gallons per hour and BTU it had no conversion for at all. Where the server asks for a unit Skip still cannot convert, the value stays in Signal K's own units and Skip names the unit in the browser console instead of failing silently. A path the server states no preference for shows its Signal K value with no unit beside it, rather than the word "unitless". The README's new **Display Units** section covers configuring all of this on the server. Fixes #536
* Signing in from Skip returns you to the page you were on, instead of leaving you on the Signal K admin screen. Skip named the return address in the redirect it sends to the login page, but under a parameter name the Signal K server does not read, so the server fell back to its own home page every time. This works with the SSO login on any server that offers one, and with the server's own login page on Signal K server 2.27.0 and later; an older server ignores the return address there, as it did before. A sign-in started inside a Freeboard-SK panel now returns to Skip's normal display rather than the chrome-free panel rendering. Fixes #554
...
## Behavior changes
* Readings whose unit Skip previously could not resolve now follow your Signal K server's unit preferences, so some existing widgets change unit on upgrade. Until now those paths kept whatever unit the widget itself had been set to; the server's preference wins. On the Metric presets, distances the server has not categorised more specifically — cross-track error, antenna altitude, anchor distance — come through as kilometers, so a 25 m cross-track error reads 0.03 km; on the Imperial presets, depths and lengths switch to feet, distances to miles, and energy readings to BTU. Change the preset, or override a single path, in the server's **Unit Preferences** settings (see the README's Display Units section); a widget whose display scale was set against the old unit is worth re-checking.
