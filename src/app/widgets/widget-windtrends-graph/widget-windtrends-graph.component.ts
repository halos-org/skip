...
import { IGraphDatapoint } from '../../core/interfaces/graph-data.interfaces';
...
import { TimeScaleFormat } from '../../core/interfaces/graph-data.interfaces';
...
export class WidgetWindTrendsGraphComponent implements OnDestroy {
  ...
  public static readonly DEFAULT_CONFIG: IWidgetSvcConfig = {
    ...
    // it follows the server's displayUnits preference, resolved at render like widget-data-graph (and by
  ...
  private readonly historyStream = inject(HistoryGraphStreamService);
   ...
   * graph in the new unit. Mirrors widget-data-graph's reactive pathMeasure. The actual conversion and
  ...
  readonly widgetDataGraph = viewChild('widgetDataGraph', { read: ElementRef });
  ...
  /** Pending coalesced graph recompute+repaint frame id (one per animation frame across both streams). */
  ...
  private dataSourceInfo: IGraphDataSourceInfo | null = null;
  ...
  private gridBackgroundPlugin = {
    ...
    beforeDraw: (chart: Chart) => {
      ...
      // Fill exactly the graphing area where grid lines are drawn
  ...
  private centerTickPlugin = {
    ...
    afterDraw: (chart) => {
      ...
      const ds = chart.data?.datasets as unknown as { label?: string; data: IDataPointRow[] }[];
  ...
  constructor() {
    ...
    // Theme or config color changes -> restyle graph
    effect(() => {
      const theme = this.theme();
  ...
  private startWidget(): void {
    ...
    const widgetDataGraphRef = this.widgetDataGraph();
    if (!widgetDataGraphRef) return;
    const cfg = this.runtime?.options();
    ...
    this.setChartOptions();
    if (!this.chart) {
      this.chart = new Chart(widgetDataGraphRef.nativeElement.getContext('2d'), {
  ...
  private isChartReady(chart: Chart): boolean {
    const dirVals = chart.data?.datasets?.[0]?.data as (IDataPointRow[] | undefined);
    const spdVals = chart.data?.datasets?.[5]?.data as (IDataPointRow[] | undefined);
  ...
  private startStreaming(): void {
    ...
    if (spdPath) {
      const twsParams: IHistoryGraphStreamParams = { ...baseParams, path: spdPath, source: spd?.source ?? 'default' };
  ...
  private pushRowsToDatasets(rows: IGraphDatapoint[]): void {
    this.pushRowsGeneric(rows, 0, 'deg', true);
  }

  private pushRowsToSpeedDatasets(rows: IGraphDatapoint[]): void {
  ...
  private getRowValue(row: IGraphDatapoint, seriesType: 'value' | 'sma' | 'ema' | 'dema' | 'avg' | 'min' | 'max'): number | null {
    switch (seriesType) {
  ...
  private transformRows(rows: IGraphDatapoint[], seriesType: 'value' | 'sma' | 'ema' | 'dema' | 'avg' | 'min' | 'max', toUnit: string, unwrap: boolean): IDataPointRow[] {
    const vals = rows.map(row => {
      const raw = this.getRowValue(row, seriesType);
  ...
  private pushRowsGeneric(rows: IGraphDatapoint[], baseIndex: 0 | 5, toUnit: string, unwrap: boolean): void {
    const types: ('value' | 'sma' | 'avg' | 'min' | 'max')[] = ['value', 'sma', 'avg', 'min', 'max'];
    types.forEach((type, i) => {
      (this.chart.data.datasets[baseIndex + i].data as IDataPointRow[])
  ...
  /**
   * Coalesce the graph recompute + repaint into a single animation frame. The direction (-twd) and
   * speed (-tws) streams frequently emit in the same frame; without this, each emission runs the
   * full 10-dataset y recompute and a graph.update('none'), doing that work twice per frame.
   */
  ...
  private updateChartAfterDataChange() {
    ...
    const dirAvgArr = this.chart.data.datasets[2]?.data as IDataPointRow[] | undefined;
    const dirSmaArr = this.chart.data.datasets[1]?.data as IDataPointRow[] | undefined;
    const dirValArr = this.chart.data.datasets[0]?.data as IDataPointRow[] | undefined;
    ...
    if (typeof centerVal === 'number' && isFinite(centerVal)) {
      ...
      const minDs = this.chart.data.datasets[3]?.data as IDataPointRow[] | undefined;
      const maxDs = this.chart.data.datasets[4]?.data as IDataPointRow[] | undefined;
    ...
    const sAvgArr = this.chart.data.datasets[7]?.data as IDataPointRow[] | undefined;
    ...
    const dirData = this.chart.data.datasets[0].data as (IDataPointRow[]);
    const speedData = this.chart.data.datasets[5].data as (IDataPointRow[]);
    if (dirData.length > 0 || speedData.length > 0) {
      const nowTs = Date.now();
      // Recompute y for all datasets as age (ms) relative to now
      this.chart.data.datasets.forEach(ds => {
        (ds.data as IDataPointRow[]).forEach(p => {
          const ts = p.ts ?? p.y;
  ...
  ngOnDestroy(): void {
    ...
    // we need to destroy when moving Pages to remove Graph Objects
    this.chart?.destroy();
    const canvas = this.widgetDataGraph?.()?.nativeElement as HTMLCanvasElement | undefined;
