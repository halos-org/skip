...
import {
  ...
  resolveDualAxisWidgetTypeFromSeries,
  TDualAxisMetric,
  TDualAxisWidgetType,
  transformDualAxisMetricValue
} from '../../contracts/electrical-history-graph.contract';
...
type HistoryGraphDataset = ChartDataset<'line', ChartPoint[]>;

/****
 * Dialog payload used by WidgetHistoryGraphDialogComponent.
 */
export interface IWidgetHistoryGraphDialogData {
  title: string;
  widget: IWidget;
...
@Component({
  selector: 'widget-history-graph-dialog',
  imports: [ MatDialogModule, MatButtonToggleModule, MatButtonModule, MatIconModule, FormsModule],
  templateUrl: './widget-history-graph-dialog.component.html',
  styleUrl: './widget-history-graph-dialog.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class WidgetHistoryGraphDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  private readonly historyApiClient = inject(HistoryApiClientService);
  private readonly historyMapper = inject(HistoryToGraphMapperService);
  private readonly app = inject(AppService);
  private readonly destroyRef = inject(DestroyRef);
  private readonly units = inject(UnitsService);
  public readonly data = inject<IWidgetHistoryGraphDialogData>(MAT_DIALOG_DATA);
   ...
   * decoupled from the graph display window so a briefly-quiet device is not dropped from enumeration.
   */
  ...
  public async loadHistoryDatasets(): Promise<void> {
    ...
    try {
      ...
      this.pendingDatasets = datasets.filter((dataset): dataset is HistoryGraphDataset => !!dataset);
      this.datasetCount.set(this.pendingDatasets.length);
      this.tryRenderChart();
    } catch (error) {
      console.error('[WidgetHistoryGraphDialogComponent] Failed to load widget history datasets:', error);
  ...
  /**
   * Angular lifecycle hook that destroys the graph instance.
   *
   * @returns {void}
   *
   * @example
  ...
  private async buildDatasetForSeries(series: ISkipSeriesDefinition, index: number): Promise<HistoryGraphDataset | null> {
    ...
    for (const candidate of requestCandidates) {
      ...
      // candidate is skipped and the next one (or an empty graph) is tried.
  ...
  private async expandTemplateSeries(series: ISkipSeriesDefinition): Promise<ISkipSeriesDefinition[]> {
    ...
    // Enumerate concretes over the retention window, not the (possibly short) graph display window:
    // getPaths only lists paths with a sample inside the window, so a briefly-quiet device would be
    // dropped if enumerated over the display window alone. The graph data itself is still fetched over
    ...
    return matchedPaths.map((path, index): ISkipConcreteSeriesDefinition => ({
      ...
      seriesUuid: `${series.seriesUuid}:resolved:${index}`,
      path,
  ...
  private buildLegendLabels(chart: Chart): LegendItem[] {
    return Chart.defaults.plugins.legend.labels.generateLabels(chart).map(label => {
      const dataset = label.datasetIndex != null
        ? chart.data.datasets[label.datasetIndex] as HistoryGraphDataset
        : null;
  ...
  private resolveDualAxisWidgetType(): TDualAxisWidgetType | null {
    return resolveDualAxisWidgetTypeFromSeries(
