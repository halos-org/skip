...
import { WidgetWindTrendsGraphComponent } from './widget-windtrends-graph.component';
...
import type { IGraphDatapoint } from '../../core/interfaces/graph-data.interfaces';
...
const cloneDefaultPaths = (): IPathArray =>
  structuredClone(WidgetWindTrendsGraphComponent.DEFAULT_CONFIG.paths) as IPathArray;

describe('WidgetWindTrendsGraphComponent', () => {
  let fixture: ComponentFixture<WidgetWindTrendsGraphComponent>;
  ...
  const setup = async (timeScale = 'Last 30 Minutes', paths: IPathArray = cloneDefaultPaths()): Promise<void> => {
    ...
    fixture = TestBed.createComponent(WidgetWindTrendsGraphComponent);
  ...
  // The "Data acquisition…" overlay's readiness gate is internal to the graph plugin; isChartReady is
  // its testable seam. Reads the component's live graph (both share the same datasets reference).
  ...
  // The big-number unit label is drawn inside the graph plugin (which the MockChart never invokes), so
  ...
  it('graphs direction and speed backfill batches without crashing', async () => {
    const dir = new Subject();
    const spd = new Subject();
    historyMock.getBackfillThenLive
      .mockReturnValueOnce(dir)
  ...
  it('keeps TWD structural but TWS a display path, so the history dialog follows the server unit', () => {
    ...
    const paths = WidgetWindTrendsGraphComponent.DEFAULT_CONFIG.paths as IPathArray;
