...
export type GraphStatsDomain = 'scalar' | 'direction' | 'signed';

/** Per-point statistics over a rolling window (newest value last). */
export interface IGraphPointStats {
  value: number;
  sma: number;
  lastAverage: number;
  lastMinimum: number;
 ...
 */
export function windowSma(window: number[], domain: GraphStatsDomain): number {
  if (domain === 'scalar') {
 ...
 * Shared by the History-API graph live tail; mirrors the recorder's per-point statistics.
 */
export function computeWindowStats(buffer: number[], smoothingPeriod: number, domain: GraphStatsDomain): IGraphPointStats {
