...
const makeConfig = (overrides: Partial<IWidgetSvcConfig> = {}): IWidgetSvcConfig => ({
  ...WidgetDataGraphComponent.DEFAULT_CONFIG,
  datachartPath: 'self.navigation.speedOverGround',
  color: 'contrast',
  ...overrides
});

describe('WidgetDataGraphComponent', () => {
  let fixture: ComponentFixture<WidgetDataGraphComponent>;
  ...
  const setup = async (config: IWidgetSvcConfig): Promise<void> => {
    ...
    await TestBed.configureTestingModule({
      imports: [WidgetDataGraphComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(WidgetDataGraphComponent);
  ...
  it('states the graph window on the widget label, not on a time-axis title', async () => {
  ...
  it.each([
  ...
  ])('draws both axes\' ticks inside the graph in the $orientation layout', async ({ verticalChart }) => {
    ...
    // review would catch until someone opens a graph in that layout.
  ...
  it('rebuilds the chart when the orientation is toggled', async () => {
    ...
    // Swapping the orientation swaps which axis carries time. transformSeriesRows only transposes
    // rows as they arrive, so anything already buffered graphs as garbage — up to a 365-day window —
