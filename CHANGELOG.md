...
## Fixes
* Text and time fields inside a widget respond to touch again. The gesture recogniser that watches every widget for taps and long-presses was taking over the pointer as soon as you touched anything, including a form field — on iOS that stopped the field focusing at all, so tapping the Racer Timer's start-time input did nothing, or opened its picker for a fraction of a second before it closed. A gesture that starts on a field is now left alone, so the field behaves normally and a long press selects text instead of opening the widget's options.
* The Racer Timer's absolute start time works as intended. Typing a time submitted it half-finished — entering 20:40 sent 20:04, because the browser treats a time field as complete the moment the minutes have two digits; it now submits when you leave the field or press Enter. Setting a time also used to switch straight to the countdown view whether or not the race plugin accepted it, so a rejected or unanswered command looked exactly like a successful one. The widget now stays on the form until the plugin confirms, and tells you if it refuses or does not answer.
