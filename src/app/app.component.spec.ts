...
describe('AppComponent — embed read-only invariants (#216 E6)', () => {
  ...
  it('runs the config migration in the full app for an upgradeable v16 config (the update-interval gate)', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 16 });
    expect(runUpgradeSpy).toHaveBeenCalledWith(16);
  });

  it('runs the config migration in the full app for an upgradeable v17 config (the wind-family path gate)', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 17 });
    expect(runUpgradeSpy).toHaveBeenCalledWith(17);
  });

  it('runs the config migration in the full app for an upgradeable v18 config (the autopilot path gate)', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 18 });
    expect(runUpgradeSpy).toHaveBeenCalledWith(18);
  });

  it('runs the config migration in the full app for an upgradeable v19 config (the first SI step)', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 19 });
    expect(runUpgradeSpy).toHaveBeenCalledWith(19);
  });

  it('runs the config migration in the full app for an upgradeable v20 config', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 20 });
    expect(runUpgradeSpy).toHaveBeenCalledWith(20);
  });

  it('runs the config migration in the full app for an upgradeable v21 config', async () => {
    const { runUpgradeSpy } = await render({ embed: false, configUpgrade: true, configVersion: 21 });
