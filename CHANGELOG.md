...
## Fixes
* Fuel rate, and every other reading whose unit your Signal K server states as `L/h`, `km/h`, `°F`, kilometers, miles or feet, now shows in that unit instead of falling back to a raw SI number labelled "unitless" — a fuel rate reads 7.2 l/h where it used to read 0.00. Skip takes each path's display unit from the server's unit preferences, but recognised only five of the unit names the server's built-in presets actually publish, and silently gave up on the rest. All of them are recognised now, and Skip also gained the imperial gallon, imperial gallons per hour and BTU it had no conversion for at all. A path the server states no preference for still shows its raw Signal K value, but with no unit beside it rather than the word "unitless". Fixes #536
