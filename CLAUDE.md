...
- **Freeboard-SK plotter extension** (`plugin/index.js`): one `plotterExtensions` resource declares Skip's contributions — a toolbar **panel** (opens the whole app in a drawer) and, additively, **widgets** (Wind Steer as 1x1/2x2 chart tiles). A widget iframe is Skip chromeless on the `#/widget/:type` route (`SingleWidgetHostComponent`), which mounts one `widget-host2` full-bleed against the live SK session; `:type` is the widget's component selector. The route is embed-only (`embedRequiredGuard`), and under embed the dashboard is force-locked, so read-only is route-enforced with no per-instance config. Data comes from Skip's own session; the plotter-extension bus (`signalk-plotterext-bus`, via `WidgetRemovalBridge`) is used only to relay a long-press to `ui.openConfigPanel` so the host opens its remove dialog — a placed widget has no other host-side remove path. `widgets` is deliberately absent from the manifest `requires` so hosts without widget support still get the panel. To expose another control as a widget, add its selector to the manifest `widgets[]`.
