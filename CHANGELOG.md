...
* The Linear gauge runs the full length of its tile. It used to hold a fixed 3:1 shape, so a wide, short tile cut the bar's length to match its height and left most of the card empty on either side; the shape is now a limit on how thick the bar gets, not on how long it runs.
## Fixes
