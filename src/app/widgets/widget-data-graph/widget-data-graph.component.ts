import { IGraphSeriesConfig, TimeScaleFormat } from '../../core/interfaces/graph-data.interfaces';
...
interface IDataPointRow { x: number | null, y: number | null }
...
// features shimmer independently as the graph scrolls. The exact-position path draws every point
...
// Compact graph-window suffix for the widget label ("SOG (30 s)"). The legacy TimeScaleFormat members
...
export class WidgetDataGraphComponent implements OnDestroy {
  ...
  private seriesConfig: IGraphSeriesConfig | null = null;
  ...
  private pathSignature = computed<string | undefined>(() => {
    ...
    // options update leaves the old scale's refresh interval running against the live graph.
  ...
  constructor() {
    ...
    // Guard: ensure graph builds once canvas exists after initial render
    effect(() => {
      const cfg = this.runtime.options();
      const canvas = this.widgetDataGraph();
      const hasPath = this.hasPath();
      if (!cfg || !hasPath || !canvas || this.chart) return;
      untracked(() => this.rebuildForSeries(cfg));
    });
  }

  private rebuildForSeries(cfg: IWidgetSvcConfig): void {
    ...
    this.streamSub?.unsubscribe(); // Cleanup old subscription & graph data
    ...
    this.setChartOptions(cfg);
    // Always recreate graph instance on rebuild to ensure orientation/scale axis changes apply
    this.chart?.destroy();
  ...
  private setChartOptions(cfg: IWidgetSvcConfig): void {
    ...
    if (!theme || !seriesConfig || !dataSourceInfo) return;
    ...
    // Ticks drawn inside the graph area: an enabled axis then costs the graph the padding alone
    ...
    if (cfg.verticalChart) {
      this.lineChartOptions.scales = {
        y: {
          ...
          suggestedMax: "",
          time: {
            unit: seriesConfig.timeScaleFormat as TimeUnit,
            minUnit: "second",
            round: "second",
            displayFormats: {
    ...
    this.lineChartOptions.plugins = {
      ...
      legend: {
       ...
       streaming: {
        ...
        frameRate: seriesConfig.timeScaleFormat === "day" ? 5 : seriesConfig.timeScaleFormat === "hour" ? 8 : seriesConfig.timeScaleFormat === "minute" ? 15 : 30,
       }
    }
  }
  ...
  private startStreaming(): void {
    ...
    // Always set by rebuildForSeries() just before startStreaming() is called (its only caller).
    ...
    this.streamSub = this.historyStream.getBackfillThenLive(params).subscribe(emission => {
      ...
      this.historyUnavailable.set(false);
      this.handleDatapointEmission(emission, cfg);
    });
  }

  private handleDatapointEmission(dsPointOrBatch: IGraphDatapoint[] | IGraphDatapoint, cfg: IWidgetSvcConfig): void {
    if (!this.chart) return;
    if (Array.isArray(dsPointOrBatch)) {
      const valueRows = this.transformSeriesRows(dsPointOrBatch, 0);
      this.chart.data.datasets[0].data.push(...valueRows);
      if (cfg.showAverageData && this.lineChartData.datasets[1]) {
        const avgRows = this.transformSeriesRows(dsPointOrBatch, cfg.datasetAverageArray);
      ...
      const valueRow = this.transformSeriesRows([dsPointOrBatch], 0)[0];
      this.chart.data.datasets[0].data.push(valueRow);
      if (cfg.showAverageData && this.lineChartData.datasets[1]) {
        const avgRow = this.transformSeriesRows([dsPointOrBatch], cfg.datasetAverageArray)[0];
  ...
  private applyTitleAndAnnotationValues(point: IGraphDatapoint, cfg: IWidgetSvcConfig): void {
  ...
  private transformSeriesRows(rows: IGraphDatapoint[], seriesType): IDataPointRow[] {
    const cfg = this.runtime.options();
    if (!cfg) return [];
    const measure = this.unitsService.resolvePathMeasure(cfg.datachartPath ?? '');
