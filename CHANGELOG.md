...
## Fixes
* A new Compact Linear gauge starts unfiltered, so its Signal K Path list offers every path rather than only the voltage ones. It shipped with its unit filter preset to **Electrical: Volt**, which is what every other general-purpose gauge leaves at **All**, and its scale preset to 0–15 to match. New widgets now start at **All** and 0–100; gauges you have already placed keep the settings you gave them. Fixes #541
