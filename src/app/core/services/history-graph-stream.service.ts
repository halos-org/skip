 ...
 * interval (not the graph cadence) so a slow-but-alive source is not gapped. Shared by the live tail
 ...
 * (bounded only by the 30s History-API HTTP timeout) freeze the graph and drop healthy live deltas.
 */
...
/** Inputs for one graph's History-API-backed data stream. */
export interface IHistoryGraphStreamParams {
  path: string;
  source: string;
  /** Raw per-graph angle-range override; combined with the path's base unit to resolve the domain. */
...
/**
 * Trend-graph data path: History-API backfill for the initial window, then a thin SK delta-stream
 * live tail with a minimal rolling buffer and the shared stats util. When no history provider is
 * available the stream emits {@link HISTORY_UNAVAILABLE} and trend graphs degrade to a clean empty
 * state.
 */
@Injectable({ providedIn: 'root' })
export class HistoryGraphStreamService {
  private readonly history = inject(HistoryApiClientService);
  private readonly mapper = inject(HistoryToGraphMapperService);
      ...
      this.fetchBackfill(params, domain)
        ...
        .catch(err => {
          ...
          if (err instanceof HistoryRequestError) {
            // Transient backfill failure (network blip, 5xx, timeout): don't disable the graph. Ride
          ...
          console.error('[HistoryGraphStreamService] Unexpected backfill error; degrading to history-unavailable:', err);
          subscriber.next(HISTORY_UNAVAILABLE);
          subscriber.complete();
        });
  ...
  private startLive(
    ...
    let gapMarked = false;
    // Staleness is keyed to the SOURCE's observed update interval, not the graph's render cadence, so a
    ...
    sub.add(merge(path$.pipe(take(1)), sampled$).subscribe(u => {
      ...
      // Stamp with the client clock so points sit on the graph's realtime axis;
   ...
   * budget elapses (whichever comes first), so a slow provider cannot freeze the graph even under sustained
  ...
  private async reBackfill(
    ...
    try {
      ...
      // to the honest gap below rather than freeze the graph for up to the 30s HTTP timeout.
      ...
      if (!(err instanceof HistoryRequestError)) {
        console.error('[HistoryGraphStreamService] Unexpected re-backfill error:', err);
      }
    } finally {
