...
## Fixes
* Weights and areas carry a unit. A displacement in kilograms or a sail area in square metres had no conversion group in Skip at all, so the widget fell back to showing a raw number with no label — on every unit preset, metric included, and whichever unit the server asked for. Both now convert and label, with pounds and square feet available for the imperial presets. Fixes #570
