...
describe('HistoryGraphStreamService', () => {
  ...
  function make(): HistoryGraphStreamService {
    TestBed.configureTestingModule({
      providers: [
        HistoryGraphStreamService,
        { provide: HistoryApiClientService, useValue: history },
        { provide: HistoryToGraphMapperService, useValue: mapper },
        { provide: DataService, useValue: data },
        { provide: ConnectionStateMachine, useValue: { state$ } }
      ]
    });
    return TestBed.inject(HistoryGraphStreamService);
  }

  beforeEach(() => {
    path$ = new Subject<IPathUpdate>();
    // Graphs mount while already connected; the reconnect re-backfill fires only on LATER Connected
  ...
  it('emits the mapped backfill as a single batch array, one request', async () => {
    ...
    expect((first as IGraphDatapoint[]).map(p => p.data.value)).toEqual([1, 3]);
  ...
  it('derives the backfill SMA circularly for direction paths (no server :sma)', async () => {
    ...
    const params: IHistoryGraphStreamParams = { ...PARAMS, angleDomainOverride: 'direction', smoothingPeriod: 3 };
    const first = await firstValueFrom(make().getBackfillThenLive(params));
    const points = first as IGraphDatapoint[];
  ...
  it('derives a scalar backfill SMA as the arithmetic mean of the trailing window (clamps early)', async () => {
    ...
    const params: IHistoryGraphStreamParams = { ...PARAMS, smoothingPeriod: 2 };
    const first = await firstValueFrom(make().getBackfillThenLive(params));
    const points = first as IGraphDatapoint[];
  ...
  it('drops null-valued backfill points', async () => {
    ...
    const first = await firstValueFrom(make().getBackfillThenLive(PARAMS));
    expect((first as IGraphDatapoint[]).map(p => p.data.value)).toEqual([4]);
  });
  ...
  it('shifts backfill timestamps from server time into the client clock (#131 skew)', async () => {
    ...
    const batch = emissions[0] as IGraphDatapoint[];
  ...
  it('holds a fresh value then breaks the trace with a gap once the source stops updating (#131)', async () => {
    ...
    try {
      ...
      const first = emissions[emissions.length - 1] as IGraphDatapoint;
      ...
      const held = emissions[emissions.length - 1] as IGraphDatapoint;
      ...
      const gap = emissions.find(
        e => !Array.isArray(e) && !('unavailable' in e) && Number.isNaN((e as IGraphDatapoint).data.value)
      );
  ...
  it('gaps a stale cached value replayed on subscribe instead of drawing it as fresh (#131)', async () => {
    ...
    const last = emissions[emissions.length - 1] as IGraphDatapoint;
  ...
  it('on a transient HistoryRequestError, does not disable the chart — rides the live tail with no backfill seed (#130)', async () => {
    ...
    // A live delta still renders, so the graph keeps working through the blip.
    const before = Date.now();
  ...
  it('transient fallback derives the clock offset from live samples so a skewed source is not false-gapped (#130)', async () => {
    ...
    try {
      ...
      const points = emissions.filter(
        (e): e is IGraphDatapoint => !Array.isArray(e) && !('unavailable' in e)
      );
  ...
  describe('reconnect re-backfill (#85)', () => {
    ...
    it('re-fetches only the missed interval on reconnect and appends it as a batch', async () => {
      ...
      try {
        ...
        const batch = emissions.filter(e => Array.isArray(e)).pop() as IGraphDatapoint[];
    ...
    it('breaks the trace with a gap when the re-backfill fails over a long outage (no interpolation)', async () => {
      ...
      try {
        ...
        expect(Number.isNaN((last as IGraphDatapoint).data.value)).toBe(true);
      } finally {
        vi.useRealTimers();
      }
    });
    ...
    it('recovers a failed initial backfill on the first connect, without waiting for a WS drop', async () => {
      ...
      // The transient failure did not disable the graph, and the seeded=false first Connected drove a
    ...
    it('caps the live-tail hold on a slow provider — draws the gap and resumes live instead of freezing', async () => {
      ...
      try {
        ...
        expect(emissions.some(e => !Array.isArray(e) && !('unavailable' in e) && Number.isNaN((e as IGraphDatapoint).data.value))).toBe(false);
        ...
        const rendered = emissions.some(
          e => !Array.isArray(e) && !('unavailable' in e) && (e as IGraphDatapoint).data.value === 7
        );
    ...
    it('advances the seam past a drawn gap so a lagging provider cannot backfill behind it', async () => {
      ...
      try {
        ...
        state$.next(ConnectionState.Connected);
        await vi.advanceTimersByTimeAsync(0);
        expect(emissions.filter(e => !Array.isArray(e) && !('unavailable' in e) && Number.isNaN((e as IGraphDatapoint).data.value)).length).toBe(1);
        ...
        const drewBackdated = emissions.some(
          e => Array.isArray(e) && (e as IGraphDatapoint[]).some(p => p.data.value === 1)
        );
